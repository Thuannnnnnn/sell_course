...
export class ResultExamController {
  ...
  @ApiOperation({ summary: 'Submit exam and auto-generate certificate if passing' })
  @ApiResponse({
    status: 201,
    description: 'Exam submitted successfully. Certificate generated if score >= 80%.',
  })
  async submitQuiz(@Request() req, @Body() submitExamDto: SubmitExamDto) {
    const result = await this.resultExamService.submitExam(req.user.email, submitExamDto);
    
    // Return additional info about certificate generation
    return {
      ...result,
      message: result.score >= 80
  ...
  @ApiResponse({ status: 200, description: 'User exam results for the course.' })
  ...
  async getQuestionsForUser(@Param('courseId') courseId: string) {
    try {
      const questions = await this.resultExamService.getQuestionsForUser(courseId);
      return questions;
    } catch {
