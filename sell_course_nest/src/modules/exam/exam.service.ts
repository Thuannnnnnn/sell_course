...
export class ExamQuestionService {
   ...
   * 🔧 FIXED: Logic to properly handle question selection
   */
  async createExamFromQuizzes(
    courseId: string,
    examConfig?: {
      questionsPerQuiz?: number; // This now means TOTAL questions from ALL quizzes
      totalQuestions?: number;    // Optional additional limit
    ...
    // ✅ FIXED LOGIC: Proper question selection based on user requirements
    let targetQuizzes = quizzes;

    // 1. Determine which quizzes to use
    ...
    // 2. Collect ALL questions from selected quizzes
    let allAvailableQuestions: Questionentity[] = [];
    for (const quiz of targetQuizzes) {
      if (quiz.questions && quiz.questions.length > 0) {
        allAvailableQuestions.push(...quiz.questions);
      }
    }
    ...
    // 3. Shuffle all questions to ensure randomness
    allAvailableQuestions = this.shuffleArray(allAvailableQuestions);

    // 4. Apply question limits
    let selectedQuestions = allAvailableQuestions;

    // If questionsPerQuiz is specified, use it as the PRIMARY limit (total questions wanted)
    if (examConfig?.questionsPerQuiz && examConfig.questionsPerQuiz > 0) {
      const questionsToTake = Math.min(examConfig.questionsPerQuiz, allAvailableQuestions.length);
      selectedQuestions = allAvailableQuestions.slice(0, questionsToTake);
    }

    // If totalQuestions is ALSO specified, apply it as an ADDITIONAL limit
    if (examConfig?.totalQuestions && examConfig.totalQuestions > 0) {
      const finalLimit = Math.min(examConfig.totalQuestions, selectedQuestions.length);
      selectedQuestions = selectedQuestions.slice(0, finalLimit);
    }
    ...
    if (selectedQuestions.length === 0) {
      throw new BadRequestException('No questions selected with the specified configuration');
    }
