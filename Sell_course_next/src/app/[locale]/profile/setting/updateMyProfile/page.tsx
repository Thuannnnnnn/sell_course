...
//       const updateUser: User = response.data
//       alert("Profile updated successfully!");

//       const newSession = await getSession()
//       console.log("Update User: ", updateUser)
//       if (newSession?.user) {
//         await update({user: newSession.user})
//       }
...
const UpdateMyProfilePage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setUser(updatedUser); // Cập nhật ngay thông tin user trong state
      await update({ user: updatedUser }); // Cập nhật session
      alert("Profile updated successfully!");
    } catch (err: any) {
      setError(err.response?.data?.message || "Error updating profile.");
    } finally {
