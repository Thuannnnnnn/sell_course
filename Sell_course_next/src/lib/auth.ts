        ...
        try {
          ...
          if (response.data?.token) {
            return {
              ...
              gender: response.data.gender,
              birthDay: response.data.birthDay,
              avartaImg: response.data.avartaImg || "/default-avatar.svg", // Default avatar
              phoneNumber: response.data.phoneNumber,
              name: response.data.username,
              role: response.data.role,
            };
  ...
  callbacks: {
    async jwt({ token, user }) {
      if (user) {
        ...
        token.role = user.role;
        token.gender = user.gender;
        token.avartaImg = user.avartaImg || "/default-avatar.svg"; // Default avatar
        token.birthDay = user.birthDay;
        token.phoneNumber = user.phoneNumber;
      }
      return token;
    ...
    async session({ session, token }) {
      ...
      session.user.birthDay = token.birthDay;
      session.user.phoneNumber = token.phoneNumber;
      session.user.avartaImg = token.avartaImg || "/default-avatar.svg"; // Default avatar if not present
      return session;
    },
    async signIn({ user, account }) {
      ...
      if (account.type === "credentials") {
        const payload = {
          ...
          email: user.email,
          name: user.name,
          avartaImg: user.avartaImg || "/default-avatar.svg", // Ensure avatar has a default
        };
        console.log("Payload prepared for API call:", payload);
        return true;
      }
      ...
      const payload = {
        ...
        email: user.email,
        name: user.name,
        avartaImg: user.avartaImg || "/default-avatar.svg", // Default avatar if not present
      };
      console.log("Payload prepared for API call:", payload);
