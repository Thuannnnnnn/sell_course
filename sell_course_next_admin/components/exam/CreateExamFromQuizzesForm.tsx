...
export function CreateExamFromQuizzesForm({
  ...
  const maxQuestionsPerQuiz = config.includeAllQuizzes 
    ...
    : selectedQuizzesData.length > 0 
      ? Math.max(...selectedQuizzesData.map(quiz => quiz.questionCount), 0)
      : 0

  // Calculate available questions for total limit
  const availableQuestionsForTotal = config.includeAllQuizzes 
  ...
  const handleQuestionsPerQuizChange = (value: string) => {
    ...
    // Validate against maximum available questions per quiz
    if (numValue && numValue > maxQuestionsPerQuiz) {
      setError(`Questions per quiz cannot exceed ${maxQuestionsPerQuiz} (maximum available in selected quizzes)`)
      return
    }
    
    setConfig(prev => ({ 
      ...prev, 
      questionsPerQuiz: numValue 
    }))
  }
  ...
  const handleTotalQuestionsChange = (value: string) => {
    ...
    // Validate against total available questions
    if (numValue && numValue > availableQuestionsForTotal) {
      setError(`Total questions cannot exceed ${availableQuestionsForTotal} (total available questions)`)
      return
    }
    
    setConfig(prev => ({ 
      ...prev, 
      totalQuestions: numValue 
    }))
  }
  ...
  const handleSubmit = () => {
    ...
    // Additional validation for exceeding limits
    if (config.questionsPerQuiz && config.questionsPerQuiz > maxQuestionsPerQuiz) {
      setError(`Questions per quiz cannot exceed ${maxQuestionsPerQuiz}`)
      return
    }

    if (config.totalQuestions && config.totalQuestions > availableQuestionsForTotal) {
      setError(`Total questions cannot exceed ${availableQuestionsForTotal}`)
      return
    }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                type="number"
                min="1"
                max={maxQuestionsPerQuiz || 50}
                value={config.questionsPerQuiz || ''}
                onChange={(e) => handleQuestionsPerQuizChange(e.target.value)}
                placeholder="e.g., 5"
              />
              <p className="text-xs text-muted-foreground">
                ...
                {maxQuestionsPerQuiz > 0 && (
                  <span className="block text-blue-600 font-medium">
                    Maximum available: {maxQuestionsPerQuiz}
                  </span>
                )}
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                type="number"
                min="1"
                max={availableQuestionsForTotal || 200}
              ...
              <p className="text-xs text-muted-foreground">
                ...
                {availableQuestionsForTotal > 0 && (
                  <span className="block text-blue-600 font-medium">
                    Maximum available: {availableQuestionsForTotal}
                  </span>
                )}
              </p>
            </div>
