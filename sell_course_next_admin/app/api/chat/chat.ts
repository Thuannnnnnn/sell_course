...
export const GetSessionChatHistory = async (
  ...
  try {
    const response: AxiosResponse<ChatHistoryResponse> = await axios.get(
      `${process.env.NEXT_PUBLIC_API_URL}/chats/history/${sessionId}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
export const DeleteOldSessionsOfUser = async (
  userId: string,
  sessionId: string,
  token: string
) => {
  try {
    await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/chats/user/${userId}/except/${sessionId}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
  } catch (error) {
    console.error("Failed to delete old sessions of user", error);
  }
};